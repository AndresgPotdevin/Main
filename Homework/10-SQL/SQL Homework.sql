...
-- table `actor` named `description` and use the data type `BLOB` (Make sure to research the type `BLOB`, as the difference between it and `VARCHAR` 
-- are significant) 
...
SELECT DISTINCT last_name, COUNT(last_name) AS "name_total" FROM actor
GROUP BY last_name;

-- 4b. List last names of actors and the number of actors who have that last name, but only for names that are shared by at least two actors --

SELECT DISTINCT last_name, COUNT(last_name) AS "name_total" FROM actor
GROUP BY last_name HAVING name_total >= 2;
...
SELECT last_name, first_name, SUM(amount) AS total_paid_per_customer
FROM payment
